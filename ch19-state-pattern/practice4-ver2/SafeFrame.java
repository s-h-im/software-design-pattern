...
public class SafeFrame extends Frame implements ActionListener, Context {
    ...
    private Button buttonEmergencyExit = new Button("비상시 종료"); // 비상시 종료 버튼

    private State state = DayState.getInstance(); // 현재 상태
    private int currentHour = 0; // 현재 시간
    ...
    public SafeFrame(String title) {
        ...
        panel.add(buttonEmergencyExit); // 비상시 종료 버튼 추가
        // 그 패널을 배치 
        ...
        buttonEmergencyExit.addActionListener(this); // 비상시 종료 버튼 리스너 설정
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == buttonUse) {		// 금고 사용 버튼
            ...
            System.exit(0);
        } else if (e.getSource() == buttonEmergencyExit
                && state == UrgentState.getInstance()) { // 비상시 종료 버튼
            // 비상시 종료 버튼 동작 처리
            if (9 <= currentHour && currentHour < 17) {
                this.changeState(DayState.getInstance());
            } else if (currentHour < 9 || 17 <= currentHour) {
                this.changeState(NightState.getInstance());
            }
        } else {
            System.out.println("?");
        }
    ...
    // 현재 시간 얻기
    public void getCurrentHour(int hour){
        currentHour = hour;
    }
